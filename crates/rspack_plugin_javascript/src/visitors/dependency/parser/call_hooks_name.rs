...
impl CallHooksName for &str {
    ...
    F: Fn(&mut JavascriptParser, &str) -> Option<T>,
  {
    ...
    // } else {
    // call_hooks_info(IdOrString::String(id), parser, hook_call)
    ...
    F: Fn(&mut JavascriptParser, &str) -> Option<T>,
  {
    match self {
      ...
      ExportedVariableInfo::VariableInfo(v) => {
        call_hooks_info(IdOrString::Id(*v), parser, hooks_call)
      }
    }
  }
...
fn call_hooks_info<F, T>(id: IdOrString, parser: &mut JavascriptParser, hook_call: F) -> Option<T>
where
  F: Fn(&mut JavascriptParser, &str) -> Option<T>,
{
