...
impl JavascriptParserPlugin for InitializeEvaluating {
  ...
  //   // }
  //   // let a = self.evaluate_identifier(parser, ident, start, end);
  //   // dbg!(a);
  //   let info = parser.get_variable_info(ident);
  //   let expr = parser.definitions_db.get_expression_info(ident);
  //   if let Some(info) = info {
  //     // if let Some(a) = a {
  //     let exp_var_info = ExportedVariableInfo::VariableInfo(info.id());
  ...
  //     ));
  //     // }
  //     // exp_var_info.call_hooks_info(parser, |parser, for_name| {
  //     //   dbg!("🥺 for_name2", for_name);
  //     //   // drive.call(parser, expr, for_name)
  //     //   // Some(true)
  //     //   // parser.evaluate(source, error_title)
  //     // });
  //     // dbg!("🟡", variable);

  //     // let var = parser.get_variable_info(name)
  //     // return info;

  //     // let mut eval = BasicEvaluatedExpression::with_range(info.range().0, info.range().1);
  //     // eval.set_identifier(
  //     //   ident.to_string(),
  //     //   info.root_info().cloned(),
  //     //   info.get_members().cloned(),
  //     //   info.get_members_optionals().cloned(),
  //     //   info.get_member_ranges().cloned(),
  //     // );
  //     // return Some(eval);
  //   }
