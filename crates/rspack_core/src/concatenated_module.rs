...
use crate::{
  ...
  ExportInfoId, ExportInfoProvided, ExportsArgument, ExportsType, FactoryMeta, IdentCollector,
...
impl Module for ConcatenatedModule {
  ...
  fn code_generation(
    ...
    if !exports_map.is_empty() {
      ...
      if let Some(should_skip_render_definitions) = compilation
        .plugin_driver
        .concatenated_module_hooks
        .exports_definitions
        .call(&mut exports_final_names)?
      {
        if !should_skip_render_definitions {
