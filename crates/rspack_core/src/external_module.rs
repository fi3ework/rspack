...
pub struct ExternalModule {
  ...
  external_type: ExternalType,
  final_external_type: ExternalType,
...
impl ExternalModule {
  pub fn new(
    ...
    let mut final_external_type: ExternalType = external_type.clone();
    if external_type == "module-import" {
      if let Some(t) = &dependency_meta.external_type {
        final_external_type = match t {
        ...
        if presets.web {
          final_external_type = "module".to_string();
        } else if presets.web_async {
          final_external_type = "import".to_string();
          ...
          || presets.nwjs
        {
          final_external_type = "node-commonjs".to_string();
        }
      }
    }
  ...
  fn get_request_and_external_type(&self) -> (Option<&ExternalRequestValue>, &ExternalType) {
    match &self.request {
      ExternalRequest::Single(request) => (Some(request), &self.final_external_type),
      ExternalRequest::Map(map) => (
        map.get(&self.final_external_type),
        &self.final_external_type,
      ),
    }
  }
  ...
  fn get_source(
    ...
    let source = match self.final_external_type.as_str() {
...
impl Module for ExternalModule {
  ...
  fn get_concatenation_bailout_reason(
    ...
    match self.final_external_type.as_ref() {
  ...
  async fn build(
    ...
    match self.final_external_type.as_str() {
